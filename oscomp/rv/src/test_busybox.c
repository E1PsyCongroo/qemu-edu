...
void test_busybox() {
    ...
    for (int i = 0; i < sizeof(tests) / sizeof(tests[0]); i++) {
        char command[256];
        snprintf(command, sizeof(command), "eval \"busybox %s\"", tests[i]);
        
        int result = run_test((char *const[]){"busybox", "sh", "-c", command, NULL});
    ...
    // int result = run_test((char *const[]){"busybox", "sh", "-c", "eval \"busybox ash -c exit\"", NULL});
    // if (result == 0) {
